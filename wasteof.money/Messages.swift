...
struct Messages: View {
    ...
    @StateObject var messagesState: MessagesStateObject = MessagesStateObject()
    @State var page = 1
    @State var read = false
    var body: some View {
        if messagesState.unread.count < 1 {
            ProgressView().onAppear() {
                fetchMessages(token: session.token, page: page, add: false, read: read)
            }
        } else {
            NavigationStack {
                VStack {
                    List {
                        ...
                        if messagesState.last != true {
                            Button {
                                page += 1
                                fetchMessages(token: session.token, page: page, add: true, read: read)
                            } label: {
                                Text("Show more")
    ...
    func fetchMessages(token: String, page: Int, add: Bool, read: Bool) {
        ...
        let task = URLSession.shared.dataTask(with: request) { data, _, error in
            ...
            do {
                ...
                DispatchQueue.main.async {
                    if read {
                        if add {
                            messagesState.unread += api.read!
                        } else {
                            messagesState.unread = api.read!
                        }
                    } else {
                        if add {
                            messagesState.unread += api.unread!
                        } else {
                            messagesState.unread = api.unread!
                        }
                    }
                    messagesState.last = api.last
                    //print(api)
                }
            } catch {
